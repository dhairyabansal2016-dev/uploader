...
def duration(filename):
    """
    Safely get video duration using ffprobe.
    Returns 0.0 if anything goes wrong (no crash).
    """
    if not os.path.exists(filename):
        print(f"duration: File does not exist: {filename}")
        return 0.0
    ...
    try:
        result = subprocess.run(
            ...
            stderr=subprocess.PIPE,   # Separate stderr to avoid error messages in stdout
            text=True,
            timeout=15                # Prevent hanging
        )

        if result.returncode != 0:
            print(f"ffprobe failed (code {result.returncode}): {result.stderr.strip()}")
            return 0.0

        output = result.stdout.strip()
        if not output or not output.replace('.', '', 1).isdigit():
            print(f"Invalid duration output: '{output}'")
            return 0.0
        ...
        print("ffprobe/ffmpeg not installed! Install it to get real duration.")
        return 0.0
